...
> **Status / scope (foundation PR).** The two Vyper contracts below —
> `PaymentSplitter` + `SubscriptionManager`, with @onder-akkaya's security guards
> (#823 review) — are the **canonical on-chain funds layer** and land in this PR.
> They are validated (compile + 14 hermetic titanoboa tests) but **not yet
> deployed** (deploy is Dan's keyed step). The **off-chain delivery** described in
> the *Operational Instructions* / *Event Flow* sections (publisher & subscriber
> agent **containers**, webhooks) is @rcrdoh's original #823 design; per the North
> Star phasing we are re-evaluating it as a **lighter shared service** (no
> per-user containers) and deferring that build. Treat the operational sections as
> the original design reference pending that decision — see the PR description for
> the consolidation rationale and the collaboration plan.
