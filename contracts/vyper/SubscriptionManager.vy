...
# Only this address may call chargeActions(). It is the off-chain platform
# charger (the strategy-runner that meters delivered actions). Defaults to the
# deployer/owner and is re-pointable by the owner via setAuthorizedCharger.
# chargeActions debits a subscriber's prepaid deposit, so leaving it
# permissionless let anyone bill any subscriber for actions never performed.
authorized_charger: public(address)
...
def __init__(_splitter: address, _usdc: address, _flat_fee: uint256):
    ...
    self.authorized_charger = msg.sender
...
def chargeActions(sub_id: bytes32, action_count: uint256):
    assert msg.sender == self.authorized_charger, "only authorized charger"
...
@external
def setAuthorizedCharger(charger: address):
    assert msg.sender == self.owner, "only owner"
    assert charger != empty(address), "invalid charger"
    self.authorized_charger = charger
