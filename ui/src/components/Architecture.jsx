	...
	return (
		<div className="max-w-[820px] mb-7">
			...
			<p className="body mb-2">
				Archimedes turns quantitative-finance research into strategies you can
				actually inspect: generated by a multi-agent debate, admitted only
				through a statistical rigor gate, executed into a vault that stays
				yours, with every decision hashed on-chain before the trade that acts on
				it.
			</p>
...
function HeroStrip({
	...
	const synthCount = contracts?.synthetics
		...
		: undefined;
	const vaultCount = contracts?.vaults
...
function PipelineFlow() {
	return (
		<div className="card p-5 mb-7">
			...
			>
				You stay in the loop at two binding moments: reviewing the passport and
				signing the deploy steps. The agent cannot withdraw, cannot change what
				the vault may hold, and cannot trade without a prior on-chain
				commitment. If nothing clears the bar, Archimedes says so:{" "}
			...
			<div className="card-flat p-4">
				...
				>
					<strong style={{ color: "var(--text-1)" }}>
						Not one paper-derived alpha strategy clears our bar.
					</strong>{" "}
					Every curated-library candidate evaluated against the live gate has
					been rejected — that is the gate doing its job, not the library
					sitting empty.{" "}
					{leaderboardError ? (
						<span style={{ color: "var(--text-4)" }}>
							Live leaderboard count unavailable right now.
						</span>
					) : loading ? (
						<span
							style={{
						...
						<>
							...
							strategies sit on the leaderboard today.
						</>
					)}
				</p>
			</div>
...
function OnChainAuthority({ contracts, contractsError }) {
	return (
		<div className="card p-5 mb-7">
			<div className="label mb-3">
				Non-custodial by contract, not by promise
			</div>
			<p className="body mb-4">
				Your deposit lives in a vault contract on Arc that you own. The agent is
				a named address with exactly one power. The contract — not our goodwill
				— enforces the split:
			</p>
					...
					<tbody>
						{AGENT_AUTHORITY.map((row) => (
							<tr key={row.can}>
								<td>
									<span
										className="i-lucide-check-circle-2 w-3.5 h-3.5"
										style={{ color: "var(--positive)", marginRight: 6 }}
									/>
									{row.can}
								</td>
								<td>
									<span
										className="i-lucide-x-circle w-3.5 h-3.5"
										style={{ color: "var(--negative)", marginRight: 6 }}
									/>
									{row.cannot}
								</td>
							</tr>
			...
			<p className="caption mb-3" style={{ color: "var(--text-3)" }}>
				Vault creation happens in your wallet — you sign{" "}
				<code>createVault</code> and <code>setAgent</code> yourself, then
				approve → deposit → allocate. Owner: you. Agent: rebalance-only.
			...
			>
				...
				Price-oracle pushes are being refreshed following the most recent
				contract redeploy, so some live pricing views may lag. This page does
				not claim live vault NAV, minting, or burning is demonstrably working
				right now.
			</div>
		</div>
	);
...
function CommitTradeRevealLoop() {
	return (
		...
		>
			...
			<desc id="ctr-desc">
				Three-step loop: the agent commits a reasoning hash, the vault contract
				enforces that the trade cannot happen without that commitment, then the
				full trace is revealed and re-hashed on-chain to verify the match.
			</desc>
			<defs>
				<marker
					id="ctrArrow"
			...
			<g>
				...
				<text x="70" y="76" textAnchor="middle" fontSize="10" fill="#A1A1AA">
					Commit
				</text>
			</g>
			<g>
				...
				<text x="230" y="76" textAnchor="middle" fontSize="10" fill="#A1A1AA">
					Trade
				</text>
			</g>
			<g>
				...
				<text x="390" y="76" textAnchor="middle" fontSize="10" fill="#A1A1AA">
					Reveal
				</text>
			</g>
		</svg>
	...
	return (
		<div className="card p-5 mb-7">
			...
			<p className="body mb-4">
				...
				generate a hundred rationales and keep the flattering one. Archimedes
				anchors <strong>before</strong>:
			</p>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
				...
				<div className="card-flat p-3">
					...
					<div className="caption" style={{ color: "var(--text-3)" }}>
						The vault's <code>rebalance()</code> reverts unless that commitment
						exists — the ordering is enforced by the contract, not by our code
						being well-behaved.
					</div>
				</div>
				<div className="card-flat p-3">
	...
	return (
		<div className="mb-7">
			...
			<div className="card-flat p-4">
				<div className="label mb-2">Honesty note</div>
				<p className="caption" style={{ color: "var(--text-3)" }}>
					Vault principal is non-custodial today and always. Marketplace fee
					custody is being migrated to a fully non-custodial design; fee
					settlement is currently in dry-run.
				</p>
			</div>
		</div>
	);
	...
	return (
		<div className="card p-5 mb-7">
			...
			<p className="body mb-3">
				...
				trading-agent research is unreproducible — run as code in the live path.
			</p>
	...
	return (
		<div className="card p-5 mb-7">
			...
			<p className="body mb-4">
				...
				signatures, faucet USDC, no real funds at risk by design. Claims must be
				true on the live path; here is the current state, kept current from the
				system's own health surface:
			</p>
				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<LedgerStatus tone="live">Live</LedgerStatus> — you sign
								everything; testnet USDC
							</td>
						</tr>
						<tr>
