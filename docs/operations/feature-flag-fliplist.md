...
| 10 | `DOCS_SITE_ENABLED` | GitHub repo variable, unset → both jobs skip (grey, not red) | repo Settings → Variables → `DOCS_SITE_ENABLED = true`, then dispatch once | Pages source set to "GitHub Actions" and the Route 53 CNAME `docs.archimedes-arc.com → a-apin.github.io`. Full steps: [`docs/runbooks/docs-site-setup.md`](../runbooks/docs-site-setup.md). |
| 11 | `RUNNER_DEPLOY_ENABLED` | GitHub repo variable, unset → both jobs skip | repo variable → `true` | The runner infrastructure must exist first (#1065 step 2 `terraform apply` + step 3 on-chain verify). [`deploy-runners.yml`](../../.github/workflows/deploy-runners.yml) has a third layer of runtime existence checks, so an early flip no-ops loudly rather than erroring — but it is still an early flip. |
...
| `chainlink_covered` / `chainlink_covered_synths()` | a historical comment in [`backend/tests/test_asset_universe_doc.py:84`](../../backend/tests/test_asset_universe_doc.py) | **ALREADY REMOVED.** Not a flag; #834's audit section listed the `universe.py` `SyntheticSpec` field as ready for cleanup, and re-grepping on 2026-08-31 finds it gone from `universe.py` entirely. Strike the action item. |
