...
| `PAPER_ADVANCE_ENABLED` | `advance_enabled()` — [`services/paper_trading.py`](../../backend/archimedes/services/paper_trading.py), checked once per tick inside `paper_advance_loop` | **pinned `"false"` in `infra/ecs.tf` AND injected `"false"` by the CI task-def rewrite** — **code default is `"false"`**. Unset must not tick: task-def :211 (#1725 image) cloned last-good, the name was absent, the old default ON started the tick, `/health` 502'd at `PAPER_ADVANCE_STARTUP_DELAY_S`. The rewrite is the path that ships; the terraform pin is the apply-time twin so a future apply cannot undo it; the code default is the last line if both pins are missing. | **Break-glass kill switch, RE-ARMED 2026-09-01 after the cause was found and fixed ([#1632](https://github.com/a-apin/archimedes/issues/1632): concurrent corpus-load session-teardown race, fixed by #1740 — the dual-OpenSSL hypothesis was falsified). The tick runs ONLY in a child interpreter (`arm_paper_advance_for_web_tier`), so a residual C abort kills the child, never `/health`. Pull it again by flipping the same three places back to `false`.** Gates the daily paper-advance tick. Off means **paper ledgers do not advance**: track records freeze, and the deployment payload keeps serving its last mark. That is a live product claim suspended on purpose, not a dormant switch. Pulled because the tick was killing web-tier tasks — the faulthandler traceback on #1632 shows `Fatal Python error: Aborted` inside psycopg2 `do_executemany` on the OHLCV cache write in [`services/market_data_provider.py`](../../backend/archimedes/services/market_data_provider.py), reached from the replay. A C-level abort is uncatchable, so the loop's fail-soft arm could not contain it and the container died at the first tick (+`PAPER_ADVANCE_STARTUP_DELAY_S`, 240s after boot) — a cold-fleet spiral. The web process also **refuses to run `paper_advance_loop` in-process** (`arm_paper_advance_for_web_tier`): even if this flag is true, work happens only in a child interpreter so a C abort cannot take `/health`. Flip-back is an explicit `"true"` in the deploy.yml rewrite pin *and* `infra/ecs.tf` *and* the code default, after #1632 has a proven cause and a fix. terraform apply alone is overwritten by the next CI deploy. Only the falsy literals (`0`/`false`/`no`/`off`) disable; only an explicit non-falsy value enables. |
...
| `BACKTEST_REFRESH_ENABLED` | `refresh_enabled()` — [`services/backtest_scheduler.py`](../../backend/archimedes/services/backtest_scheduler.py) | unset → code default `"1"` (on); forced off under `TESTING` | The curated library's staleness-driven backtest refresh loop. Kill switch for a scheduler that otherwise runs unattended. |
...
| 6 | `KB_PIPELINE_ENABLED` | unset (deliberately, per [`infra/kb_runner.tf`](../../infra/kb_runner.tf)) | env on the scheduled kb-runner task | [#778](https://github.com/a-apin/archimedes/issues/778) / #1090 / #1092. Setting it today does **not** produce a KB: [`scripts/run_kb_pipeline.py`](../../backend/archimedes/scripts/run_kb_pipeline.py) raises "the full pipeline invocation is not yet wired". The task is also sized for skip-mode (see `variables.tf`) — flipping without resizing is a second failure. |
...
on ([#1595](https://github.com/a-apin/archimedes/pull/1595)) merged
