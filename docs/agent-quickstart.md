...
- `-c/-b /tmp/agora.jar` is the session cookie jar. One jar for the whole run; it is the
  only auth mechanism on `main` today — there is no bearer token for this API. (A scoped
  API-key lane is owner decision **D3** on
  [#1653](https://github.com/a-apin/archimedes/pull/1653) and is being built on a branch;
  until it merges, assume the cookie. See *Driving this from an MCP client instead*,
  below.)
