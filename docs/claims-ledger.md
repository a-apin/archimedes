...
| "Does Archimedes trade for me?" paper-trading split | `TRUE` | `ui/src/components/Landing.jsx:182` quotes the two-book split from [PR #1704](https://github.com/a-apin/archimedes/pull/1704#issuecomment-5493036672): `paper_daily_returns` is the graded track record; `paper_agent_trades` is an executor ledger. Neither is ERC-4626 proof. |
...
| `agent.json` `description` — "executed in a non-custodial USDC vault on the Arc testnet" | `CHANGED` | Same sentence, same fix, then narrowed again: `ui/public/.well-known/agent.json:4` now says generation is not anchored on-chain, quotes the paper-trading two-book split (`paper_agent_trades` is PR #1704, not on main), and states vault execution as roadmap, not shipped. The served twin — `/api/agent/manifest`'s `blurb`, `backend/archimedes/api/agent_manifest_routes.py:157` — carries the identical string, and `backend/tests/test_agent_manifest_static_consistency.py` asserts the two are equal and that neither says "executed in". |
...
| `agent.json` `endpoints.paper.note` — "deploy is also live and puts real capital on-chain" | `CHANGED` | Testnet faucet USDC is not real capital. Rewritten by #1650, then narrowed again: `ui/public/.well-known/agent.json` paper note quotes the two-book split from [PR #1704](https://github.com/a-apin/archimedes/pull/1704#issuecomment-5493036672) (`paper_daily_returns` vs `paper_agent_trades`) and says neither is on-chain execution proof. Vault execution is roadmap, not shipped. |
...
| Claim | Status | What backs it |
|---|---|---|
| "Vault execution reads as present tense throughout this doc ... it is roadmap, not shipped product" | `CHANGED` | Banner plus body, 2026-09-01 (`docs/user-stories.md:8`). Execute/monitor are labeled roadmap in the spine (`:109`, `:115`); CreateVaultModal is marked roadmap in the click-path mermaid; the judge happy-path no longer walks Deploy as shipped. |
