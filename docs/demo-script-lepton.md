...
| 15 | 2:20 | Strategy Passport — rigor panel | Hover the DSR panel; scroll to equity curve | "DSR p-value of 0.95 means there is less than a five-percent chance this Sharpe is purely overfitting. PBO below 0.5 means the probability of backtest overfitting is below the coin-flip line. OOS Sharpe is positive. Passes all four gates: DEPLOYABLE." | If passport shows 'pending' (returns not yet written): narrate that the gate is running and will resolve in seconds; switch to a pre-existing library strategy with a PASS badge |
...
| "Insights page: real conversion funnel (landed → generation started → ...)" | `ui/src/components/Insights.jsx` — `GET /api/metrics/funnel`; `App.jsx` emits `landed` beacon on every page load | **VERIFIED** | If funnel shows zero: say "live telemetry; counts are sparse on a fresh EC2 — the funnel infrastructure is wired" |
