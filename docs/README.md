...
| [`runbooks/cloudfront-cache-behaviour-apply.md`](runbooks/cloudfront-cache-behaviour-apply.md) | runbook | Dan Browne | 2026-09-01 | Applying a CloudFront cache-behaviour change: why merging `infra/cloudfront.tf` changes nothing on its own, the exact plan to expect for #1768 (+3 behaviours on `aws_cloudfront_distribution.main`, nothing else), the mid-list-insert rendering caveat, the post-apply invalidation and `x-cache` verification, and the signed-out-`/app` rollback. |
