...
|---|---|
| **Xia et al. 2026** — *Agentic Trading: When LLM Agents Meet Financial Markets*. ESWA. [arxiv 2605.19337](https://arxiv.org/abs/2605.19337) | The audit-grade survey our R3 reproducibility target is built against (**15/19 trading-agent papers are R0**, **0/19 reach R3**). Xia's named protocols (Outcome Embargo, Time-Aware Retrieval, Hierarchy of Truth, Source Tracking, `V_check`) are research provenance of the gate and debate, not a live vault rebalance path. See [`specs/xia-2026-protocols.md`](specs/xia-2026-protocols.md). |
...
The corpus itself is arXiv q-fin **preprints** — not peer-reviewed, and not the same
thing as the five papers above. Live counts come from `GET /health`
(`corpus_db_count` / `corpus_papers`); this file never freezes a number. How it is
built, stored, and selected from: [`corpus-architecture.md`](corpus-architecture.md).
