...
**Updated 2026-07-06 (issue #1039, P1 — "stop the bleeding").** Building the
...
build-in-CI → push to ECR → pull-on-box:
...
2. The `deploy` job (needs: `build-and-push`) OIDCs into AWS and SSMs the EC2:
   `git reset --hard origin/main` (the gitignored `.env` is **untouched**, so
   the managed-store URLs persist), sets `ECR_REGISTRY`/`IMAGE_TAG` in `.env`,
   logs the box in to ECR via its own instance role, then
   `docker compose pull && docker compose up -d --force-recreate
   --remove-orphans` — **no `docker build` ever runs on the serving host** —
   then seed/hydrate/AMM bootstrap + a `/health` check.
