...
> **Carrier-contract mismatch to resolve in the PR (verified):** the existing runners return a *single* `_CandidateResult`, but the society produces a top-10 *leaderboard*. Either (a) `run_generation` is extended to accept a `list[_CandidateResult]` from the debate runner, or (b) the runner returns the **leader** `_CandidateResult` and stashes the full leaderboard on it (e.g. a `considered_alternatives` field) for the persist tail to fan out. Pick one in the PR and state it; this is a real contract delta, not a detail.
